...
| [`lab/`](lab/) | `verify.sh` + `verify.out` — the read-only phase 1/2/3 pass, run on a real Mac (an unmanaged one, so the outputs are honest) · `triage.sh` + `triage.out` — the demonstrative troubleshooting flow (collect → classify → propose → stop at the gate) · `agent-runs/` — the evidence behind each 🔨 ledger row · `agent/run.py` — the bare-API runner, `agent/run_local_tool.py` — the same with one shell tool for rows that must act · `check_secrets.sh` — refuses a transcript with a key in it |
