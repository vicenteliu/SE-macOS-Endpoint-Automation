...
| [`lab/`](lab/) | `verify.sh` + `verify.out` — the read-only phase 1/2/3 pass, run on a real Mac (an unmanaged one, so the outputs are honest) · `agent-runs/` — the evidence behind each 🔨 ledger row · `agent/run.py` — the bare-API runner · `check_secrets.sh` — refuses a transcript with a key in it |
