...
| T.1 | ″ | ″ | ″ | ″ | ″ | `ollama:ornith-1.5:9b@lab-host` | | ⏳ — collecting a bundle needs tools (read the files, `tar`), so the **no-tools** bare-API light harness cannot measure it; unlike T.2 it has no inline form. Its local line waits on a tool-using local harness | |
...
path. T.1's local line stays ⏳ for the opposite reason: collecting a bundle needs tools, so the
no-tools path cannot measure it at all.
