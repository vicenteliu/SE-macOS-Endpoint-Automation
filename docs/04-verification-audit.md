...
was changed to pass it. Each row that fails becomes a hop when it is turned into a command, and
is ticked here when that happens.
...
Nothing in this chain is GUI-only. Every check that lacks a command has one that a named tier
could run, which is the finding: the debt is commands not yet written, not consoles that cannot
be avoided. Each 🔧 row is one hop; the order they are taken in is
